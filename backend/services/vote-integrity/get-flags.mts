import { read, write } from '@data-stores/psql'
import { decodeScopedUuidCursorWithLegacySimple, encodeScopedUuidCursor } from '@modules/pagination'
import sql from 'sql-template-strings'
import type { PageInfo } from '@voucha/types/pagination'
import type { VoteIntegrityFlag } from './create-flag.mts'
import type { IntegrityFlagStatus } from '@ts-shared/utils/moderation-catalogs'
import { VOTE_INTEGRITY_FLAG_PROJECTION } from './flag-projection.mts'

export async function getVoteIntegrityFlagByIdFromPrimary(
  id: string,
): Promise<VoteIntegrityFlag | null> {
  const query = sql`/* getVoteIntegrityFlagByIdFromPrimary */
    SELECT `
  query.append(VOTE_INTEGRITY_FLAG_PROJECTION)
  query.append(sql`
    FROM vote_integrity_flags
    WHERE id = ${id}
  `)
  const { rows } = await write(query)
  return (rows[0] as VoteIntegrityFlag) ?? null
}

export type GetVoteIntegrityFlagsOptions = {
  status?: IntegrityFlagStatus
  after?: string
  limit?: number
}

export type GetVoteIntegrityFlagsResult = {
  results: VoteIntegrityFlag[]
  page_info: PageInfo
}

export async function getVoteIntegrityFlags(
  options: GetVoteIntegrityFlagsOptions = {},
): Promise<GetVoteIntegrityFlagsResult> {
  const { status, after, limit = 25 } = options
  const clampedLimit = Math.min(Math.max(1, limit), 100)
  const cursorScope = buildVoteFlagCursorScope(status)

  let afterId: string | undefined
  if (after) {
    const cursor = decodeScopedUuidCursorWithLegacySimple(
      after,
      cursorScope,
      'Invalid cursor format',
    )
    afterId = cursor.id
  }

  const query = sql`/* getVoteIntegrityFlags */
    SELECT `
  query.append(VOTE_INTEGRITY_FLAG_PROJECTION)
  query.append(sql`
    FROM vote_integrity_flags
    WHERE TRUE
  `)

  if (status === 'pending') {
    query.append(sql` AND resolved_at IS NULL`)
  } else if (status === 'resolved') {
    query.append(sql` AND resolved_at IS NOT NULL`)
  }

  if (afterId) {
    query.append(sql` AND id < ${afterId}`)
  }

  query.append(sql` ORDER BY id DESC LIMIT ${clampedLimit + 1}`)

  const { rows } = await read(query)
  const flags = rows as VoteIntegrityFlag[]
  const hasNextPage = flags.length > clampedLimit
  const results = hasNextPage ? flags.slice(0, clampedLimit) : flags
  const lastNode = results[results.length - 1]

  return {
    results,
    page_info: {
      has_next_page: hasNextPage,
      end_cursor: hasNextPage && lastNode ? encodeScopedUuidCursor(lastNode.id, cursorScope) : null,
      start_cursor: results[0] ? encodeScopedUuidCursor(results[0].id, cursorScope) : null,
    },
  }
}

function buildVoteFlagCursorScope(status?: IntegrityFlagStatus): string {
  return JSON.stringify({
    resource: 'vote-integrity-flags',
    status: status ?? 'all',
    order: 'id-desc',
  })
}
