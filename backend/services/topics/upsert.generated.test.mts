...
describe('upsertTopic', () => {
  ...
  it('claims the canonical slug as a topic alias', async () => {
    ...
    const topic = await upsertTopic(WEB_PROVENANCE, `Canonical Upsert Alias ${random}`, slug)
  ...
  it('updates existing topic on conflict', async () => {
    ...
    const topic1 = await upsertTopic(WEB_PROVENANCE, `Original Name ${random}`, slug)
    const topic2 = await upsertTopic(WEB_PROVENANCE, `Updated Name ${random}`, slug)
  ...
  it('is idempotent - can run twice with same data', async () => {
    ...
    const topic1 = await upsertTopic(WEB_PROVENANCE, `Idempotent Topic ${random}`, slug, {
      aliases: [alias],
    })
    const topic2 = await upsertTopic(WEB_PROVENANCE, `Idempotent Topic ${random}`, slug, {
      aliases: [alias],
    })
  ...
  it('reuses an existing topic when the name already exists under a different slug', async () => {
    ...
    const topic2 = await upsertTopic(WEB_PROVENANCE, name, updatedSlug)
  ...
  it('resolves to the destination topic when the requested slug/name belongs to a merged-away source', async () => {
    ...
    const resolved = await upsertTopic(WEB_PROVENANCE, source.name, source.slug)
  ...
  it('resolves to the destination when the name matches it but the slug still belongs to its merged-away source', async () => {
    ...
    const source = await upsertTopic(
      WEB_PROVENANCE,
      `Merge Source Name ${random}`,
      `merge-source-slug-${random}`,
    )
    const destination = await upsertTopic(
      WEB_PROVENANCE,
      `Merge Dest Name ${random}`,
      `merge-dest-slug-${random}`,
    )
    await mergeTopicForTest(source.id, destination.id, user!.id)
    ...
    const resolved = await upsertTopic(WEB_PROVENANCE, destination.name, source.slug)
  ...
  it('resolves to the destination when the slug matches it but the name still belongs to its merged-away source', async () => {
    ...
    // would violate that index. Must resolve to the destination without attempting the update.
    const resolved = await upsertTopic(WEB_PROVENANCE, source.name, destination.slug)
  ...
  it('rejects when the slug and name resolve to different topics', async () => {
    ...
    const destination = await upsertTopic(
      WEB_PROVENANCE,
      `Mismatch Dest ${random}`,
      `mismatch-dest-${random}`,
    )
    await mergeTopicForTest(source.id, destination.id, user!.id)
    const other = await upsertTopic(
      WEB_PROVENANCE,
      `Mismatch Other ${random}`,
      `mismatch-other-${random}`,
    )
    ...
    await expect(upsertTopic(WEB_PROVENANCE, other.name, source.slug)).rejects.toMatchObject({
      status: 409,
    })
  })
  ...
  it('retries cleanly when concurrent upserts race on the same topic', async () => {
    ...
    const [topic1, topic2, topic3, topic4] = await Promise.all([
      upsertTopic(WEB_PROVENANCE, name, slug),
      upsertTopic(WEB_PROVENANCE, name, slug),
      upsertTopic(WEB_PROVENANCE, name, slug),
      upsertTopic(WEB_PROVENANCE, name, slug),
    ])
