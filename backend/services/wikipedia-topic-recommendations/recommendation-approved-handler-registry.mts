...
// wikipedia-topic-recommendations never imports user-import-export directly, which would
...
// Internal to wikipedia-topic-recommendations: only approve-topic-recommendation-side-effects.mts
...
export function getRegisteredRecommendationApprovedHandler(): RecommendationApprovedHandler {
  if (!registeredHandler) {
    throw createCodedError(
      500,
      'No recommendation-approved handler registered for wikipedia-topic-recommendations; @services/user-import-export must be imported for side effects before topic-recommendation approvals occur',
