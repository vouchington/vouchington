...
export function assertValidTopicRecommendationInput(
  ...
  assertTopicWikipediaPageId(input)
...
function assertTopicWikipediaPageId(
  input: CreateTopicRecommendationInput | UpdateTopicRecommendationInput,
): void {
  if (!('topic_wikipedia_pageid' in input) || input.topic_wikipedia_pageid === undefined) return
  assert(
    typeof input.topic_wikipedia_pageid === 'string' &&
      input.topic_wikipedia_pageid.trim().length > 0,
    422,
    'Topic wikipedia page ID must be a string',
  )
}
