# Wikipedia Topic Recommendations Service
...
- adapts Wikipedia-derived suggestions into the generic topic recommendation post model
...
The Wikipedia recommender does not create a separate Wikipedia table. It creates normal `topic_recommendation` posts with:

- rationale markdown that includes Wikipedia source metadata and extraction context
- proposed topic markdown derived from the Wikipedia description/extract
- primary hostname + hostnames seeded from the Wikipedia URL hostname
- `topic_wikipedia_pageid` persisted for duplicate detection across title or hostname changes
- runtime validation that rejects malformed Wikipedia metadata and requires a finite `0..1` extraction confidence before persistence
- a transaction-scoped advisory lock plus duplicate re-check in the write path so concurrent creates do not insert duplicate recommendations
...
- Duplicate checks: [check-duplicate-topic.mts](./check-duplicate-topic.mts)
- Wikipedia create adapter: [create-compatibility-recommendation.mts](./create-compatibility-recommendation.mts)
