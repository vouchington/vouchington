...
describe('index.generated', () => {
  ...
  describe('topic recommendation services', () => {
    ...
    it('updates the wikipedia page id when requested', async () => {
      const recommendation = await createTopicRecommendation(user, {
        markdown: 'Original rationale',
        topic_title: 'Wikipedia page topic',
        topic_slug: `wikipedia-page-topic-${Date.now()}`,
        topic_wikipedia_pageid: '101',
      })
      const updated = await updateTopicRecommendation(user, recommendation, {
        topic_wikipedia_pageid: '202',
      })

      expect(updated.topic_recommendation?.topic_wikipedia_pageid).toBe('202')
    })
  ...
  void (0 as unknown as typeof createRecommendation)
})
