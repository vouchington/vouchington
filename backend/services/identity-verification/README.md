...
- `onCheckoutCompletedForIdentity(eventId, eventData)` — Stripe event: transitions from `payment_pending` → `identity_pending` and creates a Stripe Identity VerificationSession; accepts `payment_status === 'paid'` or `'no_payment_required'` (zero-fee checkout)
...
- Stripe event wiring: [../stripe-event-processing/](../stripe-event-processing/)
