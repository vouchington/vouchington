import {
  overrideDynamicConfigFieldsForTest,
  deleteDynamicConfigFieldsForTest,
} from '@voucha/test-helpers/dynamic-config'
import { randomUUID } from 'node:crypto'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { verifyAndStoreAttestation } from './attestation.mts'
import { storeAppAttestChallenge } from './challenges.mts'
import { appAttestationConfig } from './config.mts'
import { loadAttestationKeyByKeyId } from './store.mts'

// Real Apple-issued development attestation from node-app-attest's own test fixture
// (test/fixtures/attestation-development.json). The challenge and attestation fields there
// are base64-encoded; keyId is passed through as raw base64 (it is itself a hash, not
// further encoded data). keyId is cryptographically fixed to this specific device key --
// it cannot be randomized like other test fixtures.
const BUNDLE_IDENTIFIER = 'io.uebelacker.AppAttestExample'
const TEAM_IDENTIFIER = 'V8H6LQ9448'
const FIXTURE_CHALLENGE = Buffer.from(
  'NmY0NmFhZWItMzk4OS00NWRiLThjMjQtNmNjODhhNzZlNzg5',
  'base64',
).toString('utf8')
const FIXTURE_KEY_ID = 's/134MbeEEZDZKCvOTf+jZgNhpoDwdXZ8cKfTym8FUg='
const FIXTURE_ATTESTATION = Buffer.from(
  '',
  'base64',
)

function testChallengeKey(label: string): string {
  return `attestation-test-${label}-${randomUUID()}`
}

async function attemptFixtureAttestation(
  challengeKey: string,
  did: string,
): Promise<{ keyId: string; environment: string }> {
  try {
    const result = await verifyAndStoreAttestation({
      challengeKey,
      keyId: FIXTURE_KEY_ID,
      did,
      attestation: FIXTURE_ATTESTATION,
    })
    return { keyId: result.keyId, environment: result.environment }
  } catch (err) {
    const status = (err as { status?: number }).status
    if (status !== 409) throw err
    // A 409 here only happens after crypto verification already succeeded --
    // insertAttestationKey runs after verifyAttestation -- and this fixture's keyId is
    // cryptographically fixed, so a prior run against this dirty DB already inserted it.
    // This exact fixture always verifies to 'development'.
    return { keyId: FIXTURE_KEY_ID, environment: 'development' }
  }
}

describe('verifyAndStoreAttestation', () => {
  beforeEach(() => {
    vi.stubEnv('APPLE_APP_ATTEST_TEAM_ID', TEAM_IDENTIFIER)
    vi.stubEnv('APPLE_APP_ATTEST_BUNDLE_ID', BUNDLE_IDENTIFIER)
    overrideDynamicConfigFieldsForTest(appAttestationConfig, {
      allow_development_attestation: true,
    })
  })

  afterEach(() => {
    vi.unstubAllEnvs()
    deleteDynamicConfigFieldsForTest(
      appAttestationConfig,
      Object.keys(appAttestationConfig.fieldTypes),
    )
  })

  it('verifies a real device attestation and stores the key', async () => {
    const challengeKey = testChallengeKey('success')
    await storeAppAttestChallenge('attestation', challengeKey, FIXTURE_CHALLENGE)

    const outcome = await attemptFixtureAttestation(challengeKey, randomUUID())
    expect(outcome).toEqual({ keyId: FIXTURE_KEY_ID, environment: 'development' })

    const stored = await loadAttestationKeyByKeyId(Buffer.from(FIXTURE_KEY_ID, 'base64'))
    expect(stored).toMatchObject({
      bundle_id: BUNDLE_IDENTIFIER,
      sign_count: 0,
      environment: 'development',
    })
  })

  it('rejects when the attestation challenge is missing or expired', async () => {
    await expect(
      verifyAndStoreAttestation({
        challengeKey: testChallengeKey('missing-challenge'),
        keyId: 'irrelevant',
        did: randomUUID(),
        attestation: Buffer.from('not-a-real-attestation'),
      }),
    ).rejects.toMatchObject({ status: 400 })
  })

  it('rejects a replayed (already-consumed) attestation challenge', async () => {
    const challengeKey = testChallengeKey('replay')
    await storeAppAttestChallenge('attestation', challengeKey, 'unused-decoy-challenge')

    // First call consumes the challenge via Valkey GETDEL before any crypto runs, so its
    // own outcome (a 401 from the garbage attestation below) is irrelevant here.
    await verifyAndStoreAttestation({
      challengeKey,
      keyId: 'irrelevant',
      did: randomUUID(),
      attestation: Buffer.from('not-a-real-attestation'),
    }).catch(() => null)

    await expect(
      verifyAndStoreAttestation({
        challengeKey,
        keyId: 'irrelevant',
        did: randomUUID(),
        attestation: Buffer.from('not-a-real-attestation'),
      }),
    ).rejects.toMatchObject({ status: 400 })
  })
})
