...
import {
  readCopyrightStaffQueueBoundary,
...
describe('copyright staff queue pagination', () => {
  ...
  it('continues actionable cases without repeats and rejects malformed or cross-scope cursors', async () => {
    ...
    const boundary = await readCopyrightStaffQueueBoundary(created)
    const expectedIds = new Set(created.filter(id => id !== boundary.id))
    const request = createRequest()
    await request.authenticateAs(await createTestUser({ extraRoles: ['moderator'] }))

    const initial = await request
      .get('/api/v1/copyright-notices/review-queue?limit=100')
      .expect(200)
    expect(initial.body.copyright_notices.length).toBeLessThanOrEqual(100)
    expect(initial.body.page_info).toMatchObject({
      has_next_page: true,
      start_cursor: expect.any(String),
      end_cursor: expect.any(String),
    })

    const firstCursor = encodeScopedPreciseTimestampCursor(
      boundary.received_at,
      boundary.id,
      'copyright-notices:staff-queue:received-at-asc-id-asc',
    )
    ...
    for (let pageCount = 0; pageCount < 10; pageCount += 1) {
      ...
      if ([...expectedIds].every(id => pagedIds.includes(id))) break
    ...
    for (const id of expectedIds) expect(pagedIds).toContain(id)
