...
| ------------------------------------------- | -------------------------- | -------- | ------------------- | -------- | --------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `copyright_notice_submission_assessment_id` | `uuid`                     | no       |                     |          |           |           | One-to-one initial-notice assessment that authorizes this enforcement request and supplies its idempotency identity.                                   |
| `copyright_notice_id`                       | `uuid`                     | no       |                     |          |           |           | Copyright case whose current compliant initial-notice assessment authorizes target restrictions.                                                       |
...
| `state`                                     | `text`                     | no       | `'pending'::text`   |          |           |           | Outbox lifecycle state: pending, worker-claimed, or completed after target restrictions succeed or the authorizing assessment becomes non-enforceable. |
...
| `completed_at`                              | `timestamp with time zone` | yes      |                     |          |           |           | Time every required target restriction became active or the authorizing assessment became non-enforceable.                                             |
