-- Repair local/staging databases that recorded the pre-partitioned 0530 migration.
...
DO $$
DECLARE
  user_sessions_is_partitioned BOOLEAN;
BEGIN
  SELECT EXISTS (
    SELECT 1
    ...
    WHERE namespace.nspname = 'public'
      AND parent.relname = 'user_sessions'
  ) INTO user_sessions_is_partitioned;

  IF to_regclass('public.user_sessions') IS NOT NULL AND NOT user_sessions_is_partitioned THEN
    SET LOCAL lock_timeout = '5s';
    LOCK TABLE user_sessions IN ACCESS EXCLUSIVE MODE;

    ALTER TABLE user_sessions RENAME TO user_sessions__legacy_nonpartitioned;
    ALTER INDEX IF EXISTS user_sessions_pkey RENAME TO user_sessions__legacy_nonpartitioned_pkey;
    ALTER INDEX IF EXISTS idx_user_sessions__user_id_active_last_seen
      RENAME TO idx_user_sessions__legacy_user_id_active_last_seen;
    ALTER INDEX IF EXISTS idx_user_sessions__expires_at_active
      RENAME TO idx_user_sessions__legacy_expires_at_active;
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS user_sessions__default
PARTITION OF user_sessions DEFAULT;

DO $$
BEGIN
  IF to_regclass('public.user_sessions__legacy_nonpartitioned') IS NOT NULL THEN
    ...
    SELECT
      ...
      revoked_at,
      updated_at
    FROM user_sessions__legacy_nonpartitioned
