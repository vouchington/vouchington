...
import { createStripeEvent, toJobData } from '../stripe-webhook-test-fixtures.mts'
...
describe('processStripeWebhook InvoicePayment reversal reconciliation', () => {
  beforeEach(() => {
  ...
  it('reconciles a matching durable case exactly once and ignores another invoice', async () => {
    ...
    await processStripeWebhook(toJobData(unrelated))
  ...
  it('recovers a completed lost-dispute receipt once from a persisted dispute event', async () => {
    ...
    await processStripeWebhook(toJobData(stored))
    await processStripeWebhook(toJobData(stored))
    ...
    await processStripeWebhook(toJobData(await insertStripeEvent(reorderedLostEvent)))
