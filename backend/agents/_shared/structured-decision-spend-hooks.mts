...
export type StructuredDecisionSpendAttribution = {
  postId?: string
  communityId?: string
}

export interface StructuredDecisionSpendHooksDeps {
...
/**
 * Wires a structured-decision classifier's OpenRouter/Jev spend into the shared
 * `ai_usage_records` ledger and daily spend cap. `@modules/structured-decisions` itself stays
 * billing-agnostic and never imports `@services/ai-usage` -- this factory is the one place that
 * closes the two together, for any classifier dispatched through the structured-decision client
 * (originally C6 tagging, Plan #317/issue #616).
 *
 ...
 */
export function createStructuredDecisionSpendHooks(
  agentSlug: string,
  ...
  return {
    ...
    onBilledResponse: async ({ id, model, usage, requestStartedAt }) => {
      await recordUsage({
        ...
        communityId: attribution.communityId,
