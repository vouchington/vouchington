...
2. **`runToolLoopStreaming`** — use when the caller needs real-time events per tool call (chat orchestrator). Yields a `RunToolLoopStreamEvent` union and returns `RunToolLoopResult`. Subagent generators inside tools propagate `subagent_step` events automatically via `yield*`.
