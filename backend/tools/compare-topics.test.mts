...
describe('compare_topics tool — real DB', () => {
  ...
  it('includes each topic name', async () => {
    const execute = compareTopicsTool.function(user)
    const result = await execute({ topic_id_a: topicIdA, topic_id_b: topicIdB })

    expect(result.success).toBe(true)
    if (!result.success) return
    expect(result.topic_a.topic_name).toBe(`Compare Topic A ${suffix}`)
    expect(result.topic_b.topic_name).toBe(`Compare Topic B ${suffix}`)
  })

  it('accepts topic slugs and returns the resolved ids', async () => {
    const execute = compareTopicsTool.function(user)
    const result = await execute({
      topic_id_a: `compare-topic-a-${suffix}`,
      topic_id_b: `compare-topic-b-${suffix}`,
    })
  ...
  it.each([
    ['first', () => ({ topic_id_a: `missing-topic-${suffix}`, topic_id_b: topicIdB })],
    ['second', () => ({ topic_id_a: topicIdA, topic_id_b: `missing-topic-${suffix}` })],
  ])('reports a missing %s topic', async (_, args) => {
    const result = await compareTopicsTool.function(user)(args())
