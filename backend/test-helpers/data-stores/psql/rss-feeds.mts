...
export async function createLocalTestMembership(
  ...
  const projectionEndedAt =
    options.projectionEndedAt === undefined ? null : options.projectionEndedAt
  ...
  await write(sql`
    INSERT INTO membership_source_states (
      ...
      ${sourceId}, 'admin_grant', ${productId}, ${effectiveAt}, ${expiresAt},
  ...
  await write(sql`
    INSERT INTO memberships (
      ...
      ${userId}, ${sourceId}, ${productId}, ${effectiveAt}, ${expiresAt},
      ${projectionEndedAt},
      ${status === 'cancelled' ? lifecycleTimestamp : null},
      ${status === 'expired' ? lifecycleTimestamp : null},
      ${status === 'past_due' ? lifecycleTimestamp : null},
