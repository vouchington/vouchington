...
- **Remove residuals, no "legacy"** — delete prior state, dead code, and superseded paths. Backwards compatibility is not required except the expand/contract window for an already-live deploy (see [deploy decoupling](docs/overview/infrastructure/deployment.md#deploy-decoupling--independent-safety)).
