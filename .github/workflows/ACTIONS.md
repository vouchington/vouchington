...
`initialize-smoke-test.yml`) run `actions/setup-node` and then `pnpm/action-setup` with the same
`version: 12`. setup-node must come first: the action picks its pnpm bootstrap from the Node on
`PATH`, and the pnpm shims it writes run that Node. `standalone` would swap in a bundled Node.
`pnpm-activation.test.mts` enforces the order, that every call site passes only the same major
`version` (matching the Dockerfiles' `PNPM_VERSION`), that the root `package.json` has no
`packageManager` pin, and that no step activates pnpm through corepack or npm.
