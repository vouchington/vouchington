...
`version: latest-12`. setup-node must come first: the action picks its pnpm bootstrap from the Node
on `PATH`, and the pnpm shims it writes run that Node. `standalone` would swap in a bundled Node. A
bare `version: 12` would keep the release the action bundles (12.3.4 in v6.1.0), whose `pnpm dlx`
fails on ignored build scripts. `pnpm-activation.test.mts` enforces the order, that every call site
passes only the same `latest-<major>` `version` (its major matching the Dockerfiles'
`PNPM_VERSION`), that the root `package.json` has no `packageManager` pin, and that no step
activates pnpm through corepack or npm.
