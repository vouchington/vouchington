...
| `upload-full-lcov`     | Validate one lowercase hyphen-delimited suite identity, then make one GitHub artifact upload attempt of the full (pre-patch-sparsification) `coverage-full/lcov.info` as `lcov-full-<suite>`, read by the area patch-coverage gate and the informational Codecov upload. Uses `if-no-files-found: error`; callers retry once with `name-suffix: -retry` (a failed attempt can leave an unfinalized artifact that 409s on its name), then require a persisted attempt with `artifact-upload-outcome.mts`. | `suite`, `name-suffix`                                  |
