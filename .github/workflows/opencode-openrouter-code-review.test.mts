...
const path = '.github/workflows/opencode-openrouter-code-review.yml'
...
describe('OpenCode OpenRouter code review integration', () => {
  it('calls the frozen vouchington-tooling reusable workflow at a real commit SHA', () => {
    ...
    expect(workflow.jobs['opencode-openrouter-review'].uses).toMatch(callerPattern)
  })

  it('scopes the job to non-draft, same-repo, non-bot pull requests, excluding the closed action', () => {
    const job = workflow.jobs['opencode-openrouter-review']
    ...
    expect(job.if).toContain("vars.OPENCODE_OPENROUTER_CODE_REVIEW_ENABLED == 'true'")
  })
  ...
  it('passes the expected provider, model, and runner inputs, with a 15-minute timeout', () => {
    const withInputs = workflow.jobs['opencode-openrouter-review'].with ?? {}
  ...
  it('forwards the OpenRouter secret under code_review_api_key', () => {
    expect(workflow.jobs['opencode-openrouter-review'].secrets).toEqual({
      code_review_api_key: '${{ secrets.OPENROUTER_FREE_API_KEY }}',
    })
  })
  ...
  it('uses a concurrency group distinct from the sibling OpenCode/Zen caller so a stall in one never holds the other', () => {
  ...
  it('caps the job to 1 concurrent execution repo-wide via a fixed FIFO fleet-admission group', () => {
    expect(workflow.jobs['opencode-openrouter-review'].concurrency).toEqual({
      group: 'opencode-openrouter-review-fleet-admission',
      queue: 'max',
