...
Canonical local-command list is in [docs/development/tests.md](../../docs/development/tests.md); CI workflow/config/coverage details are in [docs/development/ci.md](../../docs/development/ci.md). This table records the command form each workflow should use.
...
Use the underlying command unless a package script owns repository policy. The Selene script owns tracked Lua discovery, so CI and local checks call it directly. For package binaries (tools in `node_modules/.bin`), use `pnpm exec <cmd>` — this finds the locally installed binary without relying on package-script PATH injection. Examples:

| Instead of                                                         | Use                                                                                                                                                               |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm run oxlint`                                                  | `pnpm exec oxlint --type-aware --deny-warnings`                                                                                                                   |
| `pnpm run oxfmt:check`                                             | `pnpm exec oxfmt --check`                                                                                                                                         |
| `selene --config selene.toml backend/services/jwt-session/scripts` | `pnpm run selene`                                                                                                                                                 |
