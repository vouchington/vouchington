...
| `pnpm exec jscpd`                       | `pnpm run jscpd` — a repository policy wrapper ([`run-jscpd.mts`](../../static-code-analysis/run-jscpd.mts)): resolves the merge-base, scans tracked files only, and fails only on new clones. Raw `jscpd` skips those rules |
