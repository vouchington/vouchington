...
describe('PR patch coverage', () => {
  it('keeps patch coverage as the sole PR coverage gate', () => {
    ...
    expect(jobSection(workflow, 'test-coverage')).toContain('- test-backend-modules')
    expect(jobSection(workflow, 'test-coverage')).toContain('pull-requests: write')
    expect(coverageJob).not.toContain('pull-requests: write')
