...
| [Web Tests](tests-web.yml)                                    | Reusable | `[self-hosted]` prep; `[self-hosted, Linux, Docker, Tests]` test shards             | No     | Two default web unit-test shards run on the self-hosted Tests pool; dependency/type checks, production build, and smoke moved to [Static Checks](checks-static.yml).            |
| [Web API Tests](tests-web-api.yml)                            | Reusable | `[self-hosted]` prep; `[self-hosted, Linux, Docker, Tests]` test shards             | Yes    | One default shard exercises `web/lib/api/**` directly against Postgres/Valkey and the backend. It does not build Next.js or the Cloudflare Worker and does not reserve `CPU`.   |
| [Web Integration Tests](tests-web-integration.yml)            | Reusable | `[self-hosted]` prep; `[self-hosted, Linux, Docker, Tests, CPU]` test shards        | Yes    | Built Worker → Next.js → backend fetch tests. The default is one shard because full-stack build/setup dominates; a manual override can expand its matrix later.                 |
...
| [Tooling Tests](tests-tooling.yml)                            | Reusable | `[self-hosted, Linux, Docker, Tests]`                                               | No     | Linux-owned static-analysis, dev-tool, CI-tool, Git hook, Playwright helper, and backend runtime audit tests.                                                                   |
