...
Do not use `setup-node`'s own built-in package-manager cache (its `cache:` input, or leaving `package-manager-cache` at its v7 default) on any runner — it only auto-activates for an npm `packageManager` field, so it is a silent no-op in this pnpm-only repo. Where a caller needs real caching on a hosted runner, add an explicit, correctly-keyed `actions/cache` step for the pnpm store as described above, rather than relying on `setup-node`'s cache.
