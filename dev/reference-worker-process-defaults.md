...
`./dev/tmux` starts `worker-io` (`backend/entrypoints/worker-io/serve.mts`) and `worker-cpu` (`backend/entrypoints/worker-cpu/serve.mts`) with queue lists derived from [`backend/modules/worker-queue-inventory/worker-queue-policy.json`](../backend/modules/worker-queue-inventory/worker-queue-policy.json). Every worker queue must be selected by exactly one local worker process. CPU-only queues run in `worker-cpu`; IO-capable queues run in `worker-io` unless listed in `WORKER_CPU_EXTRA_QUEUES`. `crawl_browser` connects to the Lightpanda cloud SaaS (`LIGHTPANDA_CDP_URL`, no local binary or port); set `LIGHTPANDA_TOKEN` (e.g. via `~/voucha.env`) to exercise real crawls locally. Override with env caps:
...
- `WORKER_CPU_EXTRA_QUEUES=emails,rss-feeds` — move IO-capable queues from `worker-io` to `worker-cpu`.
