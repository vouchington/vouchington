...
`LGPL-3.0-or-later` is allowlisted only for the `@img/sharp-libvips-*` prebuilt binaries that
`sharp` depends on (10 platform variants at one pinned version). Neither creates an obligation
today: nothing from either family is vendored, modified, or shipped in a committed build artifact.
