...
async function withTempDirectory(run: (root: string) => Promise<void>): Promise<void> {
  const root = mkdtempSync(join(tmpdir(), 'render-psql-docs-'))
  try {
    await run(root)
  } finally {
...
describe('renderPsqlDocs', () => {
  it('publishes raw and HTML schema leaves while retaining schema.md as the generated index', async () => {
    await withTempDirectory(async root => {
      const schemaDir = join(root, 'schema-snapshot')
  ...
  it('reports the missing Markdown directory before other absent schema inputs', async () => {
    await withTempDirectory(async root => {
      await expect(
        renderPsqlDocs({ outputDir: join(root, 'out'), schemaDir: join(root, 'missing') }),
  ...
  it('reports a missing schema.json after discovering the Markdown tree', async () => {
    await withTempDirectory(async root => {
      ...
      await expect(renderPsqlDocs({ outputDir: join(root, 'out'), schemaDir })).rejects.toThrow(
...
describe('render-psql-docs CLI', () => {
  it('runs end-to-end against the committed schema snapshot', () => {
    const root = mkdtempSync(join(tmpdir(), 'render-psql-docs-cli-'))
    try {
      const outputDir = join(root, 'out')
