...
const scriptPath = join(
  dirname(createRequire(resolve('package.json')).resolve('vouchington-tooling/package.json')),
  'scripts/allocate-browser-safe-ports.py',
)

function runAllocatorPython(program: string) {
...
describe('allocate-browser-safe-ports.py --stop wait', () => {
  ...
  it('leaves a sibling allocator owner in place during live --stop wait', async () => {
    const { stdout, stderr } = await runAllocatorPython(
      [
        'import os, tempfile',
        'from pathlib import Path',
        'hold_dir = Path(tempfile.mkdtemp(prefix="voucha-port-stop-sib-"))',
        'workspace = tempfile.mkdtemp(prefix="voucha-port-stop-sib-ws-")',
        '(hold_dir / "ports").write_text("12345")',
  ...
  it('attributes a live --stop bindable timeout with owner evidence', async () => {
    const { stdout } = await runAllocatorPython(
      [
        'import os, tempfile',
        'from pathlib import Path',
        'hold_dir = Path(tempfile.mkdtemp(prefix="voucha-port-stop-to-"))',
        'workspace = tempfile.mkdtemp(prefix="voucha-port-stop-to-ws-")',
        '(hold_dir / "ports").write_text("12345")',
        '(hold_dir / "pid").write_text(str(os.getpid()))',
  ...
  it('does not treat a sibling allocator as success for --release', async () => {
    const { stdout } = await runAllocatorPython(
      [
        'import tempfile',
        'from pathlib import Path',
        'hold_dir = Path(tempfile.mkdtemp(prefix="voucha-port-rel-"))',
