...
describe('githubGet', () => {
  ...
  it('retries on 429 using the Retry-After header, then succeeds', async () => {
    const mockFetch = vi
      .fn<typeof fetch>()
  ...
  it('falls back to exponential backoff without a Retry-After header', async () => {
    const mockFetch = vi
      .fn<typeof fetch>()
...
describe('deleteArtifact', () => {
  ...
  it('returns deleted on a 2xx response', async () => {
    vi.stubGlobal(
      'fetch',
  ...
  it('returns failed on a non-retryable error status', async () => {
    vi.stubGlobal(
      'fetch',
...
describe('listArtifactsPage', () => {
  ...
  it('returns the artifacts array on success', async () => {
    vi.stubGlobal(
      'fetch',
...
describe('getRunConclusion', () => {
  ...
  it('fetches and caches the conclusion of a completed run', async () => {
    ...
    await expect(getRunConclusion(REPO, TOKEN, 1, cache)).resolves.toBe('success')
    expect(mockFetch).toHaveBeenCalledOnce() // second call served from cache
  })
