...
import { runnerPortPolicy } from './runner-port-policy.mts'
...
const hostLockScriptPath = resolve('ci/with-host-lock.sh')
...
export async function withSynthesizedSlotHostLock<T>(body: () => Promise<T>): Promise<T> {
  const markerDir = await mkdtemp(join(tmpdir(), 'voucha-host-lock-marker-'))
