...
describe('download-with-diagnostics', () => {
  ...
  async function makeFakeCurl(script: string) {
    const dir = await mkdtemp(join(tmpdir(), 'voucha-download-curl-'))
    testDirs.push(dir)
  ...
  async function runDownload(options: {
    ...
    const dir = await mkdtemp(join(tmpdir(), 'voucha-download-test-'))
    testDirs.push(dir)
    ...
    const script = [
      ...
      'source ./ci/download-with-diagnostics.sh',
...
`
  ...
  it('tolerates a summary-write failure without masking the download error', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'voucha-download-summary-'))
    testDirs.push(dir)
