...
describe('allocate-browser-safe-ports.py hold mode', () => {
  ...
  async function startHolder(count: number): Promise<{
    ...
    const { stdout } = await execFile(
      'python3',
      [scriptPath, String(count), '--hold', '--hold-dir', holdDir, '--workspace', workspace],
      { env: isolatedHoldEnv() },
    )
    const ports = parsePorts(stdout)
    expectOutsideReservedSlice(ports)
    return { ports, holdDir, workspace }
  }
  ...
  it('reaps a leftover holder for the same workspace only', async () => {
    ...
    const secondHoldDir = await mkdtemp(join(tmpdir(), 'voucha-port-hold-'))
    cleanups.push(async () => {
    ...
    await expect(listenAvailable(other.ports[0], '127.0.0.1')).resolves.toBe(false)
  })

  it('resolves a relative hold directory before daemonizing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'voucha-port-hold-rel-'))
    const workspace = await mkdtemp(join(tmpdir(), 'voucha-port-hold-ws-'))
