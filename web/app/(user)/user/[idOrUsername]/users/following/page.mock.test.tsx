...
import UserFollowingUsersRoute from './page'
...
describe('UserFollowingUsersRoute', () => {
  it('renders the following page, threading the current viewer from the parallel fetch', async () => {
    ...
    const result = await UserFollowingUsersRoute({
      params: Promise.resolve({ idOrUsername: 'alice' }),
    })

    render(result)
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Following Users')
