...
import MyVideosViewedPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyVideosViewedPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyVideosViewedPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(
      'redirect:/login',
    )
  ...
  it('renders the page heading when authenticated', async () => {
    ...
    render(await MyVideosViewedPage({ searchParams: Promise.resolve({}) }))
