...
import MyVideosHiddenPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyVideosHiddenPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyVideosHiddenPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(
      'redirect:/login',
    )
  ...
  it('renders the page heading when authenticated', async () => {
    ...
    render(await MyVideosHiddenPage({ searchParams: Promise.resolve({}) }))
