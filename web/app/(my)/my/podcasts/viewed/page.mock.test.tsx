...
import MyPodcastsViewedPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyPodcastsViewedPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyPodcastsViewedPage()).rejects.toThrow('redirect:/login')
  ...
  it('renders the page heading when authenticated', async () => {
    ...
    render(await MyPodcastsViewedPage())
    expect(screen.getByTestId('bookmark-page-header')).toBeDefined()
  })

  it('renders UserViewedRssFeedRoute with podcast feedType', async () => {
    mockRequireCurrentUser.mockResolvedValue(baseUser)
    render(await MyPodcastsViewedPage())
