...
import MyPodcastEpisodesHiddenPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyPodcastEpisodesHiddenPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(
      MyPodcastEpisodesHiddenPage({ searchParams: Promise.resolve({}) }),
  ...
  it('renders the page heading when authenticated', async () => {
    ...
    render(await MyPodcastEpisodesHiddenPage({ searchParams: Promise.resolve({}) }))
