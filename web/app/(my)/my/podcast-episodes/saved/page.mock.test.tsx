...
import MyPodcastEpisodesSavedPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyPodcastEpisodesSavedPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyPodcastEpisodesSavedPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(
      'redirect:/login',
    )
  ...
  it('renders the page heading when authenticated', async () => {
    ...
    render(await MyPodcastEpisodesSavedPage({ searchParams: Promise.resolve({}) }))
