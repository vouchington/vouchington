...
import MyUsersMutedPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyUsersMutedPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyUsersMutedPage()).rejects.toThrow('redirect:/login')
  ...
  it('renders data-pw wrapper when authenticated', async () => {
    ...
    const { container } = render(await MyUsersMutedPage())
    expect(container.querySelector('[data-pw="my-users-muted-page"]')).toBeTruthy()
  })
})
