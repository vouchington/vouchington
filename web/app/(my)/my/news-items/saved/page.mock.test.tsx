...
import MyNewsItemsSavedPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyNewsItemsSavedPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyNewsItemsSavedPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(
      'redirect:/login',
    )
  ...
  it('renders the page heading when authenticated', async () => {
    ...
    render(await MyNewsItemsSavedPage({ searchParams: Promise.resolve({}) }))
