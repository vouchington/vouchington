...
import MyNewsItemsHiddenPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyNewsItemsHiddenPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyNewsItemsHiddenPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(
      'redirect:/login',
    )
  ...
  it('renders the page heading when authenticated', async () => {
    ...
    render(await MyNewsItemsHiddenPage({ searchParams: Promise.resolve({}) }))
