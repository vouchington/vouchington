...
import MyTopicsBlockedPage from './page'

const baseUser = { id: 'user-1', username: 'alice' }

describe('MyTopicsBlockedPage', () => {
  beforeEach(() => {
  ...
  it('redirects to /login when not authenticated', async () => {
    ...
    await expect(MyTopicsBlockedPage()).rejects.toThrow('redirect:/login')
  ...
  it('renders data-pw wrapper when authenticated', async () => {
    ...
    const { container } = render(await MyTopicsBlockedPage())
    expect(container.querySelector('[data-pw="my-topics-blocked-page"]')).toBeTruthy()
  })
})
