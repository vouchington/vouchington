...
describe('CopyrightStaffQueue', () => {
  it('renders private case facts and every pending decision', () => {
    ...
    expect(screen.getByLabelText('Target 1 outcome')).toHaveValue('confirm')
    expect(screen.getByLabelText('Target 2 outcome')).toHaveValue('confirm')
    expect(screen.getAllByRole('option', { name: 'Keep restriction' })).toHaveLength(2)
    expect(screen.getAllByRole('option', { name: 'Reverse restriction' })).toHaveLength(2)
    expect(screen.getByRole('checkbox', { name: 'Legal hold target 1' })).toBeChecked()
    expect(screen.getByRole('checkbox', { name: 'Legal hold target 2' })).toBeChecked()
