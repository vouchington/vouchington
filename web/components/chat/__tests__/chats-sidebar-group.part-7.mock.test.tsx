  ...
  it('uses the same continuation action for automatic and manual loading', async () => {
    ...
    mockGetMyConversationsClient
      .mockResolvedValueOnce({
        results: [conversation('conv-1', 'First')],
        page_info: makePageInfo(true, 'cursor-1'),
      })
      .mockResolvedValueOnce({
        results: [conversation('conv-2', 'Second')],
        page_info: makePageInfo(true, 'cursor-2'),
      })
      .mockResolvedValueOnce({
        results: [conversation('conv-3', 'Third')],
    ...
    await waitFor(() =>
      expect(mockGetMyConversationsClient).toHaveBeenCalledWith({
        after: 'cursor-1',
        limit: 20,
      }),
    )
