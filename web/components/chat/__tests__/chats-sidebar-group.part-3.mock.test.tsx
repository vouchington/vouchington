...
describe('ChatsSidebarGroup', () => {
  ...
  it('stops retrying the initial conversation fetch until the route changes', async () => {
    vi.useFakeTimers()
    mockGetMyConversationsClient.mockRejectedValue(new Error('persistent failure'))
    ...
    expect(mockGetMyConversationsClient).toHaveBeenCalledTimes(1)
    ...
    expect(mockGetMyConversationsClient).toHaveBeenCalledTimes(2)

    await act(async () => {
      await vi.advanceTimersByTimeAsync(10_000)
      await Promise.resolve()
    })
    expect(mockGetMyConversationsClient).toHaveBeenCalledTimes(3)
    ...
    expect(mockGetMyConversationsClient).toHaveBeenCalledTimes(4)
  })
})
