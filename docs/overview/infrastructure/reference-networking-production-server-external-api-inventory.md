...
| ---------------------------------- | --------------------------------------- | ----------------------------------------------------------------- | ---- | ------------------------- |
| Sentry backend ingest              | `o4507688154824704.ingest.us.sentry.io` | API direct                                                        | Yes  | **Allowed, exact host**   |
...
| OpenAI API                         | `api.openai.com`                        | API moderation defaults proxied; other backend callers are direct | No   | No                        |
...
| Sentry web/edge/lambda ingest      | `o4507688154824704.ingest.us.sentry.io` | Next.js server, Cloudflare Worker, Lambdas                        | Yes  | N/A; separate runtimes    |

The Cloudflare Worker `/monitoring` tunnel accepts only the web and Worker Sentry project IDs. The
backend project sends directly to the IPv6-capable Sentry host; allowing it through the tunnel would
create a circular Worker-to-backend error-reporting dependency.
