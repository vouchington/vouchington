...
| `SENTRY_TUNNEL_PREVIOUS_WEB_DSN`     | Optional retiring browser Sentry DSN. During rotation, set it to the old value while `SENTRY_WEB_DSN` holds the new value; the tunnel and web CSP accept both only while the replacement is valid, then remove it after browser rollout |
...
| `DEV_WEBSOCKET_PROXY`                | Set to `'true'` only in local `.dev.vars` to proxy `/_next/hmr` WebSockets to a localhost `WEB_ORIGIN`; production and non-local origins always reject WebSocket upgrades                                                               |
