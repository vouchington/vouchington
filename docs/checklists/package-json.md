...
- **Do not patch dependencies** — `.patch` and `.diff` files plus pnpm `patchedDependencies` and `allowUnusedPatches` are forbidden. Fix the underlying problem upstream, or file and link a Vouchington issue labeled `dependencies`.
...
- **`"license": "UNLICENSED"`** — every workspace `package.json` is `private: true` and never published, so it carries `"license": "UNLICENSED"` per npm's own convention for unpublished packages; this is registry metadata, not a rights statement — see [why, in CONTRIBUTING.md](../../CONTRIBUTING.md#why-workspace-packagejson-files-say-license-unlicensed). The `package-json-required-fields` rule in [`.no-mistakes.yml`](../../.no-mistakes.yml) enforces the field for scoped manifests.
- **Dependency license gate** — a new or changed dependency's license is checked against an allow/deny policy by `dependency-license-policy`; see [the policy note](../../static-code-analysis/README.md#dependency-license-policy) before adding a dependency under a copyleft license.
