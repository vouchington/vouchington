...
- **Local composite actions require a checkout** — `uses: ./.github/actions/<name>` only resolves once `$GITHUB_WORKSPACE` has been populated by an `actions/checkout` step; a job with no checkout cannot call one. Jobs that preserve a persistent workspace use the bounded `actions/checkout` → workspace-cleaner restore → `clean-workspace` sequence. Do not add that sequence to a job deliberately kept checkout-free (for example, a required fan-in gate) merely to deduplicate inline script; weigh the real runner-demand and registry-reachability cost first. Do not add an unconditional whole-workspace `find`/`chown`/`chmod` traversal before checkout. Repair the producer or runner isolation, or use a targeted post-failure repair when logs demonstrate the need.
