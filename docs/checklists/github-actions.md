...
- **Runner preference** — never use GitHub-hosted labels such as `ubuntu-*`, `windows-*`, or `macos-*`. Use the repo self-hosted, Ubicloud, or CodeBuild runner labels allowed by `runner-policy.test.mts`; use purpose-labelled self-hosted arrays (`[self-hosted, Tests]`, `[self-hosted, Linux, Docker]`, etc.) when the job runs on self-hosted runners.
- **Runner label changes** — update `RUNNERS.md § Runner Types` (each token must appear; enforced by `runner-docs-sync.test.mts`), the relevant grouped inventory reference's Runner column, and `runner-policy.test.mts` exact arrays — or `ephemeral-runner-policy.test.mts`'s closed `ubicloud-standard-2` set when the job in question is one of those six (CodeBuild routing lives in `runner-codebuild.test.mts`). Note: the canonical `Workflow automation map` Mermaid does **not** encode runner labels.
...
- **Concurrency** — every workflow must declare a concurrency group. PR workflows normally cancel in-progress; per-unit main workflows use `cancel-in-progress: false`. A readiness transition may deliberately queue behind same-SHA draft work when its ready path reuses successful results and a failed draft leaves a full ready rerun; keep the exception action-specific and assert that code-changing PR events still cancel.
