...
- **Cache the pnpm store and Playwright browsers, nothing else** — every job runs on a fresh, ephemeral runner, so `setup-node-pnpm`/`setup-backend` cache the resolved `pnpm store path --silent` directory and `setup-playwright` caches `~/.cache/ms-playwright`, both via a SHA-pinned `actions/cache` step keyed on `runner.os`, `runner.arch`, and `hashFiles('pnpm-lock.yaml')` (the Playwright cache omits `restore-keys` so a stale browser build fails loudly at install time instead of silently restoring). Never cache `node_modules` directly, and never enable `actions/setup-node`'s built-in package-manager `cache:` input (it cannot be pinned to a commit SHA and cannot cover Playwright browsers). Enforced by `cache-policy.test.mts`.
