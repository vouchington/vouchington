...
| `[vitest-pool]: Timeout terminating threads worker` then minutes of silence until the GHA step timeout, often with missing `coverage/lcov.info` | Tooling threads pool could not SIGKILL a worker blocked in no-mistakes `analyzeProject` / host lock (run [34785415668](https://github.com/jonathanong/filaments/actions/runs/34785415668), [#11686](https://github.com/jonathanong/filaments/issues/11686)). `i18n-extract-codemod` now uses forks. `ci/tooling-test-runner.mts` dumps `[vitest-teardown-overrun]` and SIGKILLs the process group after terminate-worker plus grace, post-summary teardown silence, or a 6-minute wall clock — not after a per-file summary. stdout and stderr are framed independently, so a split marker still matches when the other stream emits between chunks. |
