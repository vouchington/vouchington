...
instrumentation below (historical issue #8940) exists to make that cause observable without another
round of add-instrumentation-and-wait.
...
| No sentinel line, no diagnostic report, `forks without an exit sentinel` > 0                                                                    | Unhandleable death — SIGKILL (host/container OOM-killer or an external `kill -9`) or SIGSEGV/SIGBUS before the sentinel could run. No host-pressure telemetry is collected on GitHub-hosted runners — the self-hosted-only `Host pressure diagnostics` step and `ci/host-pressure-diagnostics.sh` were removed once runners stopped being shared/persistent. If this signature recurs, capture cause evidence appropriate to GitHub-hosted, single-job-per-VM runners as new tracked follow-up work rather than restoring the deleted script. |
...
| `[vitest-pool]: Timeout terminating threads worker` then minutes of silence until the GHA step timeout, often with missing `coverage/lcov.info` | Tooling threads pool could not SIGKILL a worker blocked in no-mistakes `analyzeProject` / host lock (historical run 34785415668 and issue #11686). `i18n-extract-codemod` now uses forks. `ci/tooling-test-runner.mts` dumps `[vitest-teardown-overrun]` and SIGKILLs the process group after terminate-worker plus grace, post-summary teardown silence, or a 12-minute wall clock — not after a per-file summary. stdout and stderr are framed independently, so a split marker still matches when the other stream emits between chunks.   |
...
rewrite). The repo's root `vitest` pin is already on 5; Vitest 5 did not restore the discarded
fork exit code/signal, and `pool: 'threads'` is still not a drop-in for backend projects — see
...
#8940). Historical issue #7016 is an uncounted sibling of #6807 — same fingerprint, with historical
PR #7017 broadening the transient-retry matcher — and belongs in this chain even though earlier
revisions of this section omitted it. Historical issue #9088 is deliberately **not** part of this
