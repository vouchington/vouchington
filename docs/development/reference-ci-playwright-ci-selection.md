...
`max(1, ceil(selected runnable spec count × 6.1 seconds / 350 seconds))`. Full-suite selections,
planner fail-open fallbacks, and manual runs use the same formula with the complete runnable
Playwright spec count. The 6.1 seconds per spec and 350-second execution budget are allocation
heuristics, not a per-spec SLA: 6.1 seconds is the hosted-runner test-step time per spec with three
Playwright workers. The execution budget leaves room for fixed per-shard build/startup overhead so
each job targets seven to eight minutes under the ten-minute ceiling. The selector rejects totals
outside GitHub's 1–256 matrix range; [Sharding](../../.github/workflows/reference-sharding.md) owns
the override inputs.
...
The current full suite resolves to six shards. Trusted PRs may add the credentialed
