...
| `pnpm exec *` / `pnpm run *` / `pnpm --dir *` / `pnpm install` / `pnpm test`                                                                    | The local dev loop (unit/integration tests against local Postgres/Valkey, `next build`, Playwright) needs broad filesystem writes and network reach. The matching Claude exclusions and Codex allows bypass the OS sandbox for these families. `pnpm --filter *`, `pnpm dlx *`, and `corepack pnpm *` have neither a Claude exclusion nor a Codex allow: a variable package name between `--filter` and the subcommand cannot stop at a safe subcommand with a trailing wildcard, and would also unsandbox `dlx`/`add`; `dlx` can execute an arbitrary registry package, and `corepack pnpm` covers all pnpm subcommands. They use the ordinary sandboxed path. |
