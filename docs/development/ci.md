...
CI selects the full job set - Playwright, credentialed Playwright, Storybook, web-integration, PR Docker image builds, and the full Vitest suite - on every pull-request run, regardless of draft state. Related static and application failures suppress downstream PR and merge-group work; Docker image validation starts only after the required `tests` check succeeds. Main push workflows run independently with `cancel-in-progress: false`. There is no cross-run producer-result reuse or per-PR opt-in: the `playwright:full` and `vitest:full` labels do nothing (CI never reads PR labels), and a draft PR selects the same jobs as a ready PR.
