...
- Local development runs both **`worker-io`** and **`worker-cpu`** (`dev/tmux` defaults to `--max-old-space-size=3072` / `UV_THREADPOOL_SIZE=8`, unconstrained by ECS task limits). Every queue must be selected in exactly one local worker.
