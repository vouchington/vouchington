...
`gh-readonly-queue/main/pr-{{PR_NUMBER}}-<base sha>`. List them with
`gh api "repos/{{REPOSITORY}}/actions/runs?event=merge_group&per_page=100"` and keep the runs whose
`head_branch` starts with `gh-readonly-queue/main/pr-{{PR_NUMBER}}-`. The newest group's failed or
...
the PR is a same-repository branch; for a fork, classify from CI evidence alone. Check whether the same
...
2. **A flaky test.** The failure is nondeterministic and not caused by this pull request. Search
   ...
   fix PR from `main` that closes it. When the fix adds or tightens an assertion, first show that
