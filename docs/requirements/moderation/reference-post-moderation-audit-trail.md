...
| `community_post_reviews` | One-to-one with posts                | Tracks `approved_at`, `rejected_at`, `unpublished_at`. Migration: `backend/data-stores/psql/migrations/0140-00-00-communities-publications.sql` (lines 211–228) |
...
- `openai_omni_moderation_flagged` — when `TRUE`, hides the post from search for non-owner/non-admin users
