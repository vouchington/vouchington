...
// Vitest itself never loads this copy: it resolves the same literal relative to that project's own
// `root: 'web'` override, landing on web/test-helpers/vitest.setup.storybook-browser-guard.mts
// instead (see that file). Both copies must stay a one-line re-export with no logic of their own.
